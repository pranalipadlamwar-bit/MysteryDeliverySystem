...
#     report[agent]["total_distance"] = round(total_distance, 2)
...
# # Store warehouse, agent, and package data separately
# warehouses = data["warehouses"]
# agents = data["agents"]
# packages = data["packages"]


# # Create an empty report dictionary
# # This will store delivery statistics for each agent
...
#     # Get warehouse name for current package
#     warehouse_name = package["warehouse"]

#     # Get warehouse coordinates
#     warehouse_location = warehouses[warehouse_name]

#     # Variables to track nearest agent
#     nearest_agent = None
#     shortest_distance = float("inf")

#     # Compare all agents to find nearest one
#     for agent, agent_location in agents.items():
...
#     # Get package destination coordinates
#     destination = package["destination"]
...
#     # Total delivery trip distance
#     # (agent -> warehouse) + (warehouse -> destination)
#     total_trip = shortest_distance + warehouse_to_destination

#     # Update report for selected agent
#     report[nearest_agent]["packages_delivered"] += 1

#     # Add trip distance to agent's total distance
#     report[nearest_agent]["total_distance"] += total_trip
...
#     # Total packages delivered by current agent
#     packages_delivered = report[agent]["packages_delivered"]

#     # Total distance traveled by current agent
#     total_distance = report[agent]["total_distance"]

#     # Efficiency formula:
#     # (packages delivered / total distance) * 100
...
# # Find the best agent based on highest efficiency
# best_agent = max(
#     report,
#     key=lambda a: report[a]["efficiency"]
# )

# # Store best agent in report
# report["best_agent"] = best_agent


# # Display final report on screen
# print(json.dumps(report, indent=4))


# # Save final report into report.json file
# with open("report.json", "w") as file:
#     json.dump(report, file, indent=4)
